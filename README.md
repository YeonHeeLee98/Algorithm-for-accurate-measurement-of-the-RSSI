...
- **알고리즘 개발을 위한 데이터 교정**
  ...
  - **노이즈 기준 설정**
    ...
    - RSSI 값 수신시 장애물, 이상치 등 노이즈로 판단되는 기준을 설정

    -  RSSI 값 측정시 N회 이내에 노이즈 기준과 부합하는 값이 발생되면 해당 값의 교정은 RSSI 값 측정에 영향을 미치지 않도록 설정

      </br>

  - **비정상적인 값인 이상치 예외처리**

    - RSSI 수신시 비정상적인 값이 반복되어 출력될 경우 해당 값을 예외처리

    - 비정상적인 값은 음수, 0으로 설정하여 해당 값은 예외처리(제거 및 수정)

      </br>

  - **딜레이 구간 설정 및 평균 값 계산**

    - RSSI 값 수신시 일정한 딜레이 구간(Parameter)을 설정
    - 해당 구간의 RSSI 값을 측정한 뒤 구간의 평균을 계산하여 거리 측정 값을 산출
